...
app.get('/', function (req, res) {
    let cat = new Promise(function(resolve, reject){
        con.query(
            "select id,slug,name, cost, image, category from (select id,slug,name,cost,image,category, if(if(@curr_category != " +
            "category, @curr_category := category, '') != '', @k := 0, @k := @k + 1) as ind   from goods, ( select @curr_category := " +
            "'' ) v ) goods where ind < 3",
...
app.get('/admin-order', function (req, res) {
     ...
     from_unixtime(date,"%y-%m-%d %h:%m") as human_date,
      ...
      ON shop_order.user_id = user_info.id    
       ORDER BY id DESC`, function (error, result, fields) {
                if (error) throw error;
                console.log(result);
...
function saveOrder(data, result) {
    ...
    let sql;
    sql = "INSERT INTO user_info (user_name, user_phone, user_email, address) VALUES ('"+data.username+"', '"
    ...
    con.query(sql, function (error, resultQuery) {
        ...
        date = new Date()/1000;
        for (let i = 0; i < result.length; i++) {
            sql = "INSERT INTO shop_order (date, user_id, goods_id, goods_cost, goods_amount, total) VALUES (" + date + "," + userId + ","
        + result[i]['id'] + "," + result[i]['cost'] + "," + data.key[result[i]['id']] + "," + data.key[result[i]['id']]*result[i]['cost']+")";
            con.query(sql, function (error, resultQuery) {
                if (error) throw error;
...
async function sendMail(data, result) {
    ...
    let mailOption = {
        from: '<lisenk0v@yandex.ru>',
        to : "lisenk0v@yandex.ru," + data.email,
        subject : "Lite shope order",
        text : "Hello world",
        html : res
